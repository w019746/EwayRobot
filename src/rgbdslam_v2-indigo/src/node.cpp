...
MatchingResult Node::matchNodePair(const Node* older_node)
{
  ...
  try{
    ...
    if (mr.all_matches.size() < min_matches){
        ...
        if(found_transformation && (fabs(tfTransDiff.getOrigin().getX())>0.1 || fabs(tfTransDiff.getOrigin().getY())>0.1 || fabs(tfTransDiff.getOrigin().getZ())>0.1 || fabs(tfTransDiff.getRotation().getAngle())>0.05))
        {
